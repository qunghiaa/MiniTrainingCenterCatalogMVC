...
public IActionResult Stats()
{
    ...
    return View(new CourseStatsViewModel
    {
        ...
        FullCourses = courses.Count(x =>
            x.EnrolledStudents >= x.Capacity),
        AvailableCourses = courses.Count(x =>
            x.EnrolledStudents < x.Capacity),
        TotalRevenue = courses.Sum(x =>
...
{
    return Content(
        "Mini Training Center MVC Running");
}
...
public IActionResult LowSeat()
{
    var data = _service.GetAll()
        .Where(c =>
            _service.IsLowSeat(c))
        .ToList();
